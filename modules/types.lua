...
--- @field front_label? fun(self: TheFamilyTab, card: Card): { text?: string, colour?: table, scale?: number } | { remove: true } Function which returns: config for displaying text on a card, or table with `remove = true` for cleaning up
--- @field popup? fun(self: TheFamilyTab, card: Card): { name?: table, description?: table[] } | { definition_function: fun(): { definition: table, config: table } } | { remove: true } Function which returns config for displaying popup on hover, or function which returns custom definition for it, or table with `remove = true` for cleaning up. Rerenders when tab is (de)selected, use `card.highlighted` to determine is tab selected
...
--- @field alert? fun(self: TheFamilyTab, card: Card): table | { definition_function: fun(): { definition: table, config: table } } | { remove: true } Function which returns: config for vanilla `create_UIBox_card_alert()` function, or function which returns custom definition for it, or table with `remove = true` for cleaning up
